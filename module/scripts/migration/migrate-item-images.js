...
export const MigrateItemImages = async function () {
  ...
  for (const actor of actorsList) {
    ...
    for (const item of actorItems) {
      // Check if there are any instances of /systems/vtm5ec/assets/icons/powers/ in the actor data
      if (countInstances(item.img, 'systems/vtm5ec/assets/icons/powers/') > 0) {
        hasFixedItems = true
        ...
        const updatedItem = {
          ...
          ...item.toObject(),
          img: item.img.replace(/\/systems\/vtm5e\/assets\/icons\/powers\//, 'systems/vtm5ec/assets/icons/items/')
        }
